...
const generateJwt = (userId, userLogin, dictPrivilegePrivId) => {
    return jwt.sign(
        {userId, userLogin, dictPrivilegePrivId},
...
class UserController {
    ...
    async addUser(req, res, next) {
        ...
        if (candidate) {
            return next(ApiError.badRequest('User with such userLogin already exists'))
        }
        ...
        const user = await UserShelter.create({userFIO, userLogin, userCheck: hashPassword, dictPrivilegePrivId})
        console.log(user)
        console.log(userFIO, userLogin, userCheck, dictPrivilegePrivId)
        const token = generateJwt(user.userId, user.userLogin, user.dictPrivilegePrivId)
    ...
    async login(req, res, next) {
        ...
        let comparePassword = bcrypt.compareSync(userCheck, user.userCheck)
        if (!comparePassword) {
    ...
    async check(req, res) {
        ...
        const token = generateJwt(req.user.userId, req.user.userLogin, req.user.dictPrivilegePrivId)
        return res.json({token})
    }
}
